...
class CommandParseTest(CommandTestCase):
    ...
    def test_char(self, filename="test_char.csv"):
        ...
        if pyversion == 2:
            self.assertEqual(unicode(item.uom), "pi图e")
        else:
            self.assertEqual(str(item.uom), "pi图e")
    ...
    def test_char2(self, filename="test_char2.csv"):
        ...
        self.command(filename, "csvimport.Item", "country=KE(Country|code)")
        item = self.get_item(
            "Cet élément est utilisé par quelqu'un d'autre et ne peux être modifié"
        )
        self.assertEqual(
            item.description,
        ...
        if pyversion == 2:
            self.assertEqual(unicode(item.uom), "删除当前图片")
        else:
    ...
    def test_number(self, filename="test_number.csv"):
        ...
        """
        errs = [
            "row 0: Column quantity = -23, less than zero so set to 0",
            "row 4: Column quantity = 1e+28 more than the max integer 9223372036854775807 sqlite may error with big integers so rounded down",
            "row 5: Column quantity = Not_a_Number is not a number so is set to 0",
            "row 6: Column quantity = nan is not an integer so is set to 0",
        ]
        self.command(
